...
class CustomKeyPage extends Component<Props> {
    ...
    onBack() {
        if (this.changeValues.length > 0) {
            Alert.alert('Remind', 'Do you want to save the change?',
                [
                    {
                        text: 'No', onPress: () => {
                            NavigationUtil.goBack(this.props.navigation)
    ...
    render() {
        const {theme} = this.params;
        let title = this.isRemoveKey ? 'Delete Tag' : 'Select Tag';
        title = this.params.flag === FLAG_LANGUAGE.flag_language ? 'Select Language' : title;
        let rightButtonTitle = this.isRemoveKey ? 'Delete' : 'Save';
